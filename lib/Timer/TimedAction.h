...
extern "C" {
  typedef std::function<void(void)> CallbackTimer;
}

class TimedAction {

  public:
    TimedAction(unsigned long interval,CallbackTimer function);
    TimedAction(unsigned long prev,unsigned long interval, CallbackTimer function);
